...
object Lecture extends Db.DbEntity[Lecture] {
  ...
  def fromDb(rs: ResultSet): List[Lecture] = {
    ...
    while (rs.next()) lb.append(Lecture(rs.getString("lecture_id"), rs.getString("fullname"), rs.getDouble("ects"),
...
case class Lecture(id:String, fullname:String, ects:Double, teacher:String,
