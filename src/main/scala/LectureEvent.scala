...
object LectureEvent extends Db.DbEntity[LectureEvent] {
  ...
  def fromDb(rs: ResultSet): List[LectureEvent] = {
    ...
    while (rs.next()) lb.append(LectureEvent(rs.getString("lecture_event_id"), rs.getDate("date_start"), rs.getDate("date_end"),
                                             rs.getString("description"), rs.getString("lecture"), rs.getString("group"),
                                             rs.getString("classroom")))
    lb.toList
  }

  def insertSql: String = "insert into dbo.lecture_event (lecture_event_id, date_start, date_end, description, lecture, group, classroom)" +
                          "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
...
case class LectureEvent(id:String, from:Date, to:Date, description:String, lecture:String,
